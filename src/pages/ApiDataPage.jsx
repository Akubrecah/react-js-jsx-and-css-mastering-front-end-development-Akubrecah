  ...
  useEffect(() => {
    const getPosts = async () => {
      try {
        setLoading(true);
        setError(null);
        const { data, totalCount } = await fetchPosts(page, 12);
        setPosts(data);
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200 min-h-screen transition-colors duration-500">
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-4xl font-bold text-center mb-8">Fetched API Data</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredPosts.map(post => (
                <Card key={post.id} className="flex flex-col">
                  ...
                  <p className="text-gray-600 dark:text-gray-400 flex-grow">{post.body}</p>
                </Card>
              ))}
            </div>
